...
function handleMySQLError(err) {
  ...
  if (msg.includes("max_connections_per_hour") || msg.includes("too many connections") || msg.includes("user limit reached") || msg.includes("resource limit") || code === "ER_USER_LIMIT_REACHED" || code === "ER_CON_COUNT_ERROR" || code === "PROTOCOL_CONNECTION_LOST" || code === "ECONNREFUSED") {
...
app.get("/api/db/:table", async (req, res) => {
  ...
  if (mysqlRes.success && Array.isArray(mysqlRes.rows) && mysqlRes.rows.length > 0) {
    ...
    if (table === "santri") {
      ...
      finalData2 = unpackLembaga(finalData2);
    }
