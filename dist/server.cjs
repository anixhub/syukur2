...
function handleMySQLError(err) {
  ...
  if (msg.includes("max_connections_per_hour") || msg.includes("too many connections") || code === "ER_USER_LIMIT_REACHED" || code === "ER_CON_COUNT_ERROR") {
    console.warn(">>> Hostinger MySQL Rate Limit / Connection Limit reached. Using persistent memory store fallback.");
    mySQLThrottleUntil = Date.now() + 1e4;
  }
}
